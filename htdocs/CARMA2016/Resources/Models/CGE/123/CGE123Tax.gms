...
set exp / benchmark, freetrade, revneutral /;
...
option nlp=minos5;
model1.iterlim = 1000; model1.optfile=4;
Solve model1 maximizing GDP using nlp;

values(vars,exp) = endog.l(vars);

$Ontext

     In this next section we implement a free trade experiment,
     along the lines of the Devarajan et al chapter. We set the import
     tariff and export tax to zero
...
values(vars,"freetrade") = endog.l(vars);


$Ontext

     In this next section we implement an experiment, again along the
     lines of the Devarajan et al chapter.  This involves a closure
     swap, wherein we exchange an exogenous and endogenous variable
     and re-solve the model under an alternative policy regime.  The
     question is what is the increase in the domestic tax rate needed
     to keep government revenue constant, if we eliminate import taxes?
     This involves making "T" exogenous, and making "ts" endogenous.

$Offtext

*FIRST, rebenchmark the model to the base equilibrium.
...
*SECOND, implement a closure swap between T and ts, and set tm to zero
tm = 0;
endog.fx("T")=endog.l("T");
endog.up("ts")=1.00;
...
values(vars,"revneutral") = endog.l(vars);
