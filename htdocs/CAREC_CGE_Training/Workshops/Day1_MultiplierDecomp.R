setwd("/Users/Sam/Dropbox/Training/Urumqi/Multipliers")
...
#setwd(" . . /R_Materials")
SAM=read.csv("SampleSAM.csv",row.names=1)

# Convert NA elements to be equal to 0

SAM[is.na(SAM)==TRUE]=0

# Remove "total" column and row from SAM
SAM = SAM[1:(nrow(SAM)-1),1:(ncol(SAM)-1)]
...
plotMat=function(x,name){
  ...
  M_rotate[M_rotate==min(M_rotate)]=0
