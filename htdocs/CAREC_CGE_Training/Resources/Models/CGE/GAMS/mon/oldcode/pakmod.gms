...
xpeq1(i,k)$(mapc(i,k) and sigmac(k) ne inf)..

   xp(i)*pp(i)**sigmac(k) =e= ac(i,k)*x(k)*p(k)**sigmac(k) ;

xpeq2(i,k)$(mapc(i,k) and sigmac(k) eq inf)..
   pp(i) =e= p(k) ;

* ----- Equation (P-26)

peq1(k)$(sigmac(k) ne inf and x0(k) ne 0)..
   p(k)**(1-sigmac(k)) =e= sum(i$mapc(i,k),ac(i,k)*pp(i)**(1-sigmac(k))) ;

peq2(k)$(sigmac(k) eq inf and x0(k) ne 0)..
   x(k) =e= sum(i$mapc(i,k),xp(i)) ;
...
gexpeq..
   gexp =e= yf("govt") + plev*sum(h,gtrh(h)) + gtrw ;
...
tieq..
   yf("capacct") =e= sum(e,savc(e)) + sum(h,savh(h)) + savg + er*savf ;
*   yf("capacct") + yf("ginvs") + yf("inventory") =e= sum(e,savc(e)) + sum(h,savh(h)) + savg + er*savf ;
